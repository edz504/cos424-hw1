...
def tokenize_corpus(path, train=True):
    ...
    for dir in dirs:
        ...
        for f in files:
            ...
            i += 1
            if (i % 100 == 0):
              print(str(i) + " / " + str(len(files)) + " done")
    if train == True:
...
# first run this to get the training data set
main(["-p", "trec07p_data/Train", "-o", "train_emails"])

# copy the train_emails_vocab_<WCT>.txt to the /Test folder, then run this
main(["-p", "trec07p_data/Test", "-o", "test_emails", "-v", "/train_emails_vocab_100.txt"])


bow_test = read_bagofwords_dat('trec07p_data/Test/test_emails_bag_of_words_0.dat', numofemails=5000)
numpy.save("test_emails_bag_of_words_0.npy", bow_test)
numpy.savetxt("test_emails_bag_of_words_0.csv", bow_test, delimiter=",")

bow_train = read_bagofwords_dat('trec07p_data/Train/train_emails_bag_of_words_100.dat', numofemails=45000)
numpy.save("train_emails_bag_of_words_100.npy", bow_train)
